		...
		if (allocation_Info[i] == 0)
		{
			...
			for (int j = i; j < num_OF_pages_In_heap; j++)
			{
				if ((j * PAGE_SIZE) + USER_HEAP_START == (UH_UnAllocated_Space_Start) || ((j - 1) * PAGE_SIZE) + USER_HEAP_START == (UH_UnAllocated_Space_Start))
				{
					...
					{
						if ((USER_HEAP_MAX - UH_UnAllocated_Space_Start) / PAGE_SIZE < min_Satisfying_Frames_Found || min_Satisfying_Frames_Found == -1)
						{
				...
				else if (allocation_Info[j] == 0)
				{
					...
					if (j == num_OF_pages_In_heap - 1)
					{
						...
						{
							if (counter < min_Satisfying_Frames_Found || min_Satisfying_Frames_Found == -1)
							{
...
void free(void *virtual_address)
{
	...
	if ((uint32)virtual_address + allocation_Info[index_in_the_array] * PAGE_SIZE == UH_UnAllocated_Space_Start)
		UH_UnAllocated_Space_Start = (uint32)virtual_address;
