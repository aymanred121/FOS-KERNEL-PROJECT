...
#define PA_MASK 0xFFFFF000
void FreeWS(struct Env * e)
{
	for (int i = 0; i < e->page_WS_max_size; i++)
	{
		unmap_frame(e->env_page_directory, (void *)env_page_ws_get_virtual_address(e, i));
		env_page_ws_clear_entry(e, i); //Remove Entry From WS
	}

}
void freeLRU(struct Env *e, struct WS_List *list)
{
...
void freeTables(struct Env * e)
{
	for (int i = 0; i < PDX(USER_TOP); i++)
		{
			if ((e->env_page_directory[i] & PA_MASK) != 0)
			{
				uint32 PA = (e->env_page_directory[i] & PA_MASK);
				struct Frame_Info *ptr_frame_info = NULL;
...
void env_free(struct Env *e)
{
	...
	// [2] Free LRU lists
	freeLRU(e, &(e->ActiveList));
	freeLRU(e, &(e->SecondList));
